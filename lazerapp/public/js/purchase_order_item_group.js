        ...
        frappe.call({
            ...
            callback: function (res) {
                if (res.message && res.message.length > 0) {
                    ...
                    groups.push("All Item Groups"); // Optional global option
                    ...
                    // Add blinking animation to parent button
                    setTimeout(function () {
...
function open_item_selection_dialog(frm, group) {
    ...
    // Fetch total count excluding "Services" & "Carwash"
    frappe.call({
...
frappe.ui.form.on('Purchase Order', {
    refresh(frm) {
        // Only show for submitted Purchase Orders
        if (frm.doc.docstatus === 1) {
            
            // 🔹 Remove duplicate buttons (from previous refresh or ERPNext default)
            frm.page.wrapper.find('.btn:contains("Create Goods Received Note")').remove();

            // 🔹 Add standalone primary button
            frm.page.add_button(__('Create Goods Received Note'), function () {
